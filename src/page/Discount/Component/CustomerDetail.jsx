...
const CustomerDetail = () => {
    ...
    const handleSubmit = async () => {
        ...
        if(customer.password !== customer.passwordConfirm && customer.password !== "" && customer.passwordConfirm !=='') {
            return toast.current.show({severity:'info', summary: 'Password confirm', detail:'Password confirm must match with the password', life: 3000});
        }

        if(customer.password) {
            customerAPI.changeCustomerPassword(id,{newPassword: customer.password})
            .then(res =>  {
                if(res.status === 200) {
        ...
        <ContentWrapper>
            <Header>
                <div className="flex w-full">
                   {customer.first && <Avatar label={customer.first[0]} size="xlarge" customSize="7rem" />}
                    <h1 className="ml-5 font-bold">{customer.last + ' ' + customer.first}</h1>
                </div>
            </Header>
